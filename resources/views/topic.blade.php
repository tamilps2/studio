            ...
            <div class="nav-scroller py-2 mb-2">
                <nav class="nav d-flex justify-content-between">
                    @foreach($data['topics'] as $topic)
                        <a class="p-2 text-muted text-decoration-none" href="{{ route('topic', $topic->slug) }}">{{ $topic->name }}</a>
                    @endforeach
                </nav>
            </div>
        @endif
                    ...
                    <div class="jumbotron p-4 p-md-5 text-white rounded @if(empty($post->featured_image)) bg-dark @endif"
                        ...
                        <div class="col-md-8 px-0">
                            <h1 class="font-italic font-serif">
                                <a href="{{ route('post', [$post->userMeta->username, $post->slug]) }}" class="text-white text-decoration-none">{{ $post->title }}</a>
                            </h1>
                            <p class="lead my-3">
                                <a href="{{ route('post', [$post->userMeta->username, $post->slug]) }}" class="text-white text-decoration-none">{{ $post->summary }}</a>
                            </p>
                            <p class="lead mb-0">
                                <a href="{{ route('post', [$post->userMeta->username, $post->slug]) }}" class="text-white font-weight-bold text-decoration-none">{{ __('canvas::blog.buttons.continue') }}</a>
                            </p>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="p-md-4">
                        ...
                        <ol class="list-unstyled mb-0">
                            @foreach($data['tags'] as $tag)
                                <li>
                                    <a href="{{ route('tag', $tag->slug) }}" class="text-decoration-none text-secondary">{{ $tag->name }}</a>
                                </li>
                            @endforeach
                        </ol>
                    </div>

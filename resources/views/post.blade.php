    ...
    @if($data['post']->user_id == optional(request()->user())->id)
        <div class="dropdown">
            <a href="#" id="actionDropdownMenu" class="nav-link pl-0 pr-3" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" viewBox="0 0 24 24" class="icon-cog"><path class="primary" d="M6.8 3.45c.87-.52 1.82-.92 2.83-1.17a2.5 2.5 0 0 0 4.74 0c1.01.25 1.96.65 2.82 1.17a2.5 2.5 0 0 0 3.36 3.36c.52.86.92 1.8 1.17 2.82a2.5 2.5 0 0 0 0 4.74c-.25 1.01-.65 1.96-1.17 2.82a2.5 2.5 0 0 0-3.36 3.36c-.86.52-1.8.92-2.82 1.17a2.5 2.5 0 0 0-4.74 0c-1.01-.25-1.96-.65-2.82-1.17a2.5 2.5 0 0 0-3.36-3.36 9.94 9.94 0 0 1-1.17-2.82 2.5 2.5 0 0 0 0-4.74c.25-1.01.65-1.96 1.17-2.82a2.5 2.5 0 0 0 3.36-3.36zM12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"/><circle cx="12" cy="12" r="2" class="primary"/></svg>
            </a>
                ...
                @if($data['post']->tags->count() > 0)
                    <h5>
                        @foreach($data['post']->tags as $tag)
                            <span><a href="#" class="badge badge-light p-2 text-decoration-none">{{ $tag->name }}</a></span>
                        @endforeach
                    </h5>
                @endif
            </div>
        ...
        <div class="row">
            @isset($data['next'])
                <div class="col-lg bg-light text-center px-lg-5 py-5"
                    ...
                    <h2 class="font-weight-bold font-serif my-3">
                        <a href="#" class="text-decoration-none @isset($data['next']->featured_image) text-light @else text-dark @endisset">{{ $data['next']->title }}</a>
                    </h2>
            ...
            @isset($data['random'])
                <div class="col-lg bg-light text-center px-lg-5 py-5"
                    ...
                    <a href="#" class="btn btn-sm text-decoration-none @isset($data['random']->featured_image) btn-outline-light @else btn-outline-secondary @endisset text-uppercase font-weight-bold mt-3">
                    ...
                    <h2 class="font-weight-bold font-serif my-3 @isset($data['random']->featured_image) text-white @endisset">
                        <a href="#" class="text-decoration-none @isset($data['random']->featured_image) text-white @else text-dark @endisset">{{ $data['random']->title }}</a>
                    </h2>
